...
def model3():
    ...
    regressorGRU.add(Embedding(output_dim=vocab_dim,
                        input_dim=n_symbols,
                        weights=[embedding_weights],
                        input_length=input_length))  # Adding Input Length
    # First LSTM layer with Dropout regularisation
    ...
    regressorGRU.add(Dropout(0.2))
    # The output layer
    regressorGRU.add(Dense(3,activation='sigmoid'))
    # Compiling the RNN
...
if __name__ == "__main__":
    ...
    print(x_train.shape,y_train.shape)
    train_lstm(n_symbols,embedding_weights,x_train[:1000],y_train[:1000],x_test[:100],y_test[:100])
